...
export const dependants16OverField = GovUKTextInput({
  ...
  label: 'Do you have any dependants aged 16 and over?',
  ...
  validWhen: [
    validation({
      ...
      message: 'Enter the number of dependants you have aged 16 and over, or enter \'0\' if none',
    }),
    validation({
...
export const dependants15UnderField = GovUKTextInput({
  ...
  label: 'Do you have any dependants aged 15 and under?',
  formatters: [Transformer.String.ToFloat()],
  inputType: 'number',
  classes: GovUKUtilityClasses.Input.Width2,
  validWhen: [
    validation({
      condition: Self().match(Condition.IsRequired()),
      message: 'Enter the number of dependants you have aged 15 and under, or enter \'0\' if none',
    }),
    validation({
      condition: and(Self().match(Condition.Number.IsInteger()), Self().match(Condition.Number.GreaterThanOrEqual(0))),
