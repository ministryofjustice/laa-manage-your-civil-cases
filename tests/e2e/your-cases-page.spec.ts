...
test.describe('Your cases Page', () => {
  ...
  //     test('has correct H1 text', async ({ page }: { page: Page }) => {
  //       await testPageHeading(page, config);
  //     });

  //     test('has correct sub-navigation with all case types', async ({ page }: { page: Page }) => {
  //       await testSubNavigation(page, YOUR_CASES_NAV_ITEMS);
  //     });

  //     test('has active tab as correct tab', async ({ page }: { page: Page }) => {
  //       await testActiveTab(page, config, YOUR_CASES_NAV_ITEMS);
  //     });

  //     test.describe('sorting functionality', () => {
  //       test('should display dates in descending order by default', async ({ page }: { page: Page }) => {
  //         await testDefaultSorting(page, config);
  //       });

  //       test('should handle sorting operations on click', async ({ page }: { page: Page }) => {
