...
test.describe('Edit Client Name', () => {
  ...
  test('viewing change name form should display expected elements', async ({ pages }) => {
    ...
    await assertCaseDetailsHeaderPresent(editNamePage.getPage, { withMenuButtons: false, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse', 'Third Party'], dateOfBirth: "18 Aug 1981 (45)" });
  ...
  test('cancel link should navigate back to client details', async ({ pages }) => {
    ...
    // Assert the case details header is present
    await assertCaseDetailsHeaderPresent(pages.editName.getPage, { withMenuButtons: true, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse', 'Third Party'], dateOfBirth: "18 Aug 1981 (45)" });
  ...
  test('invalid data rejected by backend should not update client details', async ({ page, pages }) => {
    ...
    // Assert the case details header is present
    await assertCaseDetailsHeaderPresent(editNamePage.getPage, { withMenuButtons: true, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse', 'Third Party'], dateOfBirth: "18 Aug 1981 (45)" });
  ...
  test.describe('PC-1854-6521 scenario', () => {
    ...
    test('save button should redirect to client details when valid data submitted', async ({ pages }) => {
      ...
      await editNamePage.navigate();
      // Assert the case details header is present
      await assertCaseDetailsHeaderPresent(editNamePage.getPage, { withMenuButtons: false, expectedName: "Walter White", expectedCaseRef: "PC-1854-6521", dateReceived: "8 Aug 2025 at", badgeTexts: ['At risk of abuse'], dateOfBirth: "12 Nov 1979 (46)" });

      await editNamePage.submitWithValidName('John Updated Smith');
      await editNamePage.expectSuccessfulSubmission();
      // Assert the case details header is present
      await assertCaseDetailsHeaderPresent(editNamePage.getPage, { withMenuButtons: false, expectedName: "John Updated Smith", expectedCaseRef: "PC-1854-6521", dateReceived: "8 Aug 2025 at", badgeTexts: ['At risk of abuse'], dateOfBirth: "12 Nov 1979 (46)" });
      ...
      await assertSummaryCardData(editNamePage.getPage, 'Client support needs', { 'British Sign Language': 'Yes', 'Language – needs interpreter': 'English', 'Other support': 'Here are some notes from the operator!' });
      // Assert third party details summary card is visible with data
      await assertSummaryCardState(editNamePage.getPage, { cardId: 'Third party contact', emptyText: 'No third party contact required', hasData: true, changeHref: '/client-details/change/third-party', removeHref: '/confirm/remove-third-party' });
      // Assert the correct data is displayed in the third party data summary card
      await assertSummaryCardData(editNamePage.getPage, 'Third party contact', { 'Name': 'Samira Patel', 'Phone number': 'Not provided', 'Email address': 'samira@patel.com', 'Address': '84 Zoo Lane, Birmingham B88 1RW', 'Relationship to client': 'Legal adviser' });
    });
    ...
    test('unchanged name triggers no change warning banner', async ({ pages }) => {
      ...
      // Assert support needs summary card is visible with no data 
      await assertSummaryCardState(pages.editName.getPage, { cardId: 'Client support needs', emptyText: 'No support needs', hasData: true, changeHref: '/client-details/change/support-need' });
      // Assert the data in the support needs summary card is correct
      await assertSummaryCardData(pages.editName.getPage, 'Client support needs', { 'British Sign Language': 'Yes', 'Language – needs interpreter': 'English', 'Other support': 'Here are some notes from the operator!' });
      // Assert third party details summary card is visible with data
      await assertSummaryCardState(pages.editName.getPage, { cardId: 'Third party contact', emptyText: 'No third party contact required', hasData: true, changeHref: '/client-details/change/third-party', removeHref: '/confirm/remove-third-party' });
      // Assert the correct data is displayed in the third party data summary card
      await assertSummaryCardData(editNamePage.getPage, 'Third party contact', { 'Name': 'Samira Patel', 'Phone number': 'Not provided', 'Email address': 'samira@patel.com', 'Address': '84 Zoo Lane, Birmingham B88 1RW', 'Relationship to client': 'Legal adviser' });
