...
test.describe('Edit Client Name', () => {
  ...
  test.describe('PC-1854-6521 scenario', () => {
    ...
    test('save button should redirect to client details when valid data submitted', async ({ pages, i18nSetup }) => {
      ...
      await editNamePage.navigate();
      // Assert the case details header is present
      await assertCaseDetailsHeaderPresent(editNamePage.getPage, { withMenuButtons: false, expectedName: "Walter White", expectedCaseRef: "PC-1854-6521", dateReceived: "8 August 2025", badgeTexts: [ 'At risk of abuse', 'Third Party', 'Translation', 'BSL'] }); 

      await editNamePage.submitWithValidName('John Updated Smith');
      await editNamePage.expectSuccessfulSubmission();
      // Assert the case details header is present
      await assertCaseDetailsHeaderPresent(editNamePage.getPage, { withMenuButtons: false, expectedName: "John Updated Smith", expectedCaseRef: "PC-1854-6521", dateReceived: "8 August 2025", badgeTexts: [ 'At risk of abuse', 'Third Party', 'Translation', 'BSL'] }); 
    });
  });
