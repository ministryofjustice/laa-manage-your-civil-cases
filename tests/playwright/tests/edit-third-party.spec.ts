...
test('viewing edit third party form should display expected elements', async ({ page, i18nSetup }) => {
  ...
  await assertCaseDetailsHeaderPresent(thirdPartyPage.getPage, { withMenuButtons: false, isUrgent: true, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 July 2025" }); 
...
test('cancel link should navigate back to client details', async ({ page, i18nSetup }) => {
  ...
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(thirdPartyPage.getPage, { withMenuButtons: true, isUrgent: true, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 July 2025" }); 
});
