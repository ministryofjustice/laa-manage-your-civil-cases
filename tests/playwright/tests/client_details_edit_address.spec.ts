...
test('viewing change address form for a closed case should display alert banner', async ({ page, i18nSetup }) => {
  ...
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(page, { withMenuButtons: false, expectedName: "Roronoa Zoro", expectedCaseRef: "PC-6667-9089", dateReceived: "6 Jan 2025 at", badgeTexts: ['At risk of abuse', 'Third Party'], dateOfBirth: "6 Sept 1982 (44)" });
...
test('save button should redirect to client details when valid data submitted', async ({ page, i18nSetup }) => {
  ...
  await expect(page).toHaveURL(clientDetailsUrl);
  // Assert contact details card data
