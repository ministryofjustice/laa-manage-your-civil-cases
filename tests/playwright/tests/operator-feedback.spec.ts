...
test('viewing operator feedback form should display expected elements', async ({ page, i18nSetup }) => {
  ...
  await assertCaseDetailsHeaderPresent(feedbackPage.getPage, { withMenuButtons: false, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 July 2025", badgeTexts: ['Urgent', 'At risk of abuse', 'Third Party'] }); 
...
test('cancel link should navigate back to client details', async ({ page, i18nSetup }) => {
  ...
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(feedbackPage.getPage, { withMenuButtons: true, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 July 2025", badgeTexts: ['Urgent', 'At risk of abuse', 'Third Party'] }); 
});
