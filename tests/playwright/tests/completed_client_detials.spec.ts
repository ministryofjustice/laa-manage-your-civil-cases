...
test('client details selected from completed cases tab has correct page elements', async ({ page, i18nSetup }) => {
  // Navigate to the client details
  await page.goto(getClientDetailsUrlByStatus('closed'));

  const completed_tag = page.getByText('Completed', { exact: true });
  ...
  await expect(completed_tag).toBeVisible();
