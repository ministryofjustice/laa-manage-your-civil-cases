...
test('viewing "about new case" form should display expected elements', async ({ page, pages, i18nSetup }) => {
  ...
  await assertCaseDetailsHeaderPresent(aboutNewCasePage.getPage, { withMenuButtons: false, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse', 'Third Party'] });
...
test('when there is only one category assigned to the provider and provider radio is selected this is displayed correctly', async ({ page, pages, i18nSetup }) => {
  ...
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(aboutNewCasePage.getPage, { withMenuButtons: false, expectedName: "Barbra white", expectedCaseRef: "PC-1122-1349", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse', 'Second opinion', 'Third Party'] });
