...
test('viewing "about new case" form should display expected elements', async ({ page, pages, i18nSetup }) => {
  ...
  await assertCaseDetailsHeaderPresent(aboutNewCasePage.getPage, { withMenuButtons: false, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse', 'Third Party'], dateOfBirth: "18 Aug 1981 (45)" });
...
test('when there is only one category assigned to the provider and provider radio is selected this is displayed correctly', async ({ page, pages, i18nSetup }) => {
  ...
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(aboutNewCasePage.getPage, { withMenuButtons: false, expectedName: "Barbra white", expectedCaseRef: "PC-1122-1349", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse', 'Second opinion', 'Third Party'], dateOfBirth: "16 Jun 1961 (65)" });
...
test('we should see error validations, when no data entered', async ({ page, i18nSetup }) => {
  ...
  const inlineErrorCategory = page.getByText('Error: Select the category of');
  const inlineErrorNote = page.getByText('Error: Enter why you want to')
  await expect(errorSummaryComponent).toBeVisible();
  await expect(inlineErrorCategory).toBeVisible();
  await expect(inlineErrorNote).toBeVisible();
});
...
test('we should see error validations, for when 2500 or more character entered', async ({ page, i18nSetup }) => {
  ...
  // Fill the notes field, and repeat the message 50 times
