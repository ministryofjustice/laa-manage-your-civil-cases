...
test('confirm button should delete third party contact and redirect to client details', async ({ page, i18nSetup }) => {
  ...
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(page, { withMenuButtons: true, expectedName: "George Gregory", expectedCaseRef: "PC-3152-7329", dateReceived: "8 Aug 2025 at", badgeTexts: ['BSL','Translation', 'At risk of abuse', 'Third Party'] });
  await page.goto(editThirdPartyURL);
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(page, { withMenuButtons: false, expectedName: "George Gregory", expectedCaseRef: "PC-3152-7329", dateReceived: "8 Aug 2025 at", badgeTexts: ['BSL','Translation', 'At risk of abuse', 'Third Party'] });
...
test('confirmation page shows warning text about removing third party contact', async ({ page, i18nSetup }) => {
  await page.goto(clientDetailsUrl);
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(page, { withMenuButtons: true, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse'] });
  await page.goto(visitUrl);
  // Assert the case details header is present
  await assertCaseDetailsHeaderPresent(page, { withMenuButtons: false, expectedName: "Jack Youngs", expectedCaseRef: "PC-1922-1879", dateReceived: "7 Jul 2025 at", badgeTexts: ['Urgent', 'At risk of abuse'] });
  await expect(page.locator('h2.govuk-heading-m')).toContainText('Remove third party?');
  await expect(page.getByText('This will permanently delete all information about the third party.')).toBeVisible();
});
