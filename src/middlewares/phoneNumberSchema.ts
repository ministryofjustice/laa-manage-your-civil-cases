  ...
  checkSchema({
      ...
      custom: {
        ...
        options: (numberInputted: string): true => {
          if (numberInputted.trim() === '') {
            throw new Error('Enter the client phone number');
          }

          const valid = isValidPhoneNumber(numberInputted, 'GB') || isValidPhoneNumber(numberInputted, 'IN');
          if (!valid) {
            throw new Error('Enter the phone number in the correct format');
          }
    ...
    notChanged: {
      ...
      custom: {
        ...
        errorMessage: "Update if the client is safe to call, update the client phone number, or select ‘Cancel’",
      },
    },
  });