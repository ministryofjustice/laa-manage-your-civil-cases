...
// import { handleEditThirdPartyValidationErrors } from '#src/scripts/helpers/formControllerHelpers.js';
...
export async function getEditClientThirdParty(req: Request, res: Response, next: NextFunction): Promise<void> {
  await handleGetEditForm(req, res, next, {
    ...
    dataExtractor: (apiData: unknown): Record<string, unknown> => {
      ...
      /**
       * Helper function to safely get array first element
       * @param {unknown} obj - Object containing selected array
       * @returns {string} First selected value or empty string
       */
      const getFirstSelectedValue = (obj: unknown): string => {
       ...
       * @param {unknown} passphraseObj - Passphrase object containing passphrase field
       * @returns {string} Passphrase value or empty string
       */
      const getPassphraseValue = (passphraseObj: unknown): string => {
      ...
      const flatData = {
        ...
        thirdPartyRelationshipToClient: getFirstSelectedValue(thirdPartyData.relationshipToClient),
        thirdPartyPassphraseSetUp: getFirstSelectedValue(thirdPartyData.passphraseSetUp),
        thirdPartyPassphrase: getPassphraseValue(thirdPartyData.passphraseSetUp)
      };
