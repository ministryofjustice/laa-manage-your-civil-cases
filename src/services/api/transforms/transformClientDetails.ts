...
export function transformClientDetailsItem(item: unknown): ClientDetailsResponse {
  ...
  const provider_assigned_at = formatLongFormDateWithShortMonth(safeString(item.provider_assigned_at));
  const provider_viewed = formatLongFormDateWithShortMonth(safeOptionalString(item.provider_viewed) ?? '');
  const provider_accepted = formatLongFormDateWithShortMonth(safeOptionalString(item.provider_accepted) ?? '');
  const provider_closed = formatLongFormDateWithShortMonth(safeOptionalString(item.provider_closed) ?? '');
